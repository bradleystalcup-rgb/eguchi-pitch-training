...
export const childProfiles = pgTable(
  "child_profiles",
  {
    ...
    hotkeyMode: text("hotkey_mode").notNull().default("left"),
    accidentalMode: text("accidental_mode").notNull().default("sharps"),
    chordSelectionAlgorithm: text("chord_selection_algorithm").notNull().default("random"),
    soundEngine: text("sound_engine").notNull().default("tone"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => ({
